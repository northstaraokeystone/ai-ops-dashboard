...
def validate_data_integrity(records: list[dict], target_col: str | None = None) -> dict:
    ...
    if imbalance_score is not None:
        score = (missing_score * weights['completeness'] + schema_score * weights['schema'] + duplicate_score * weights['duplicates'] + imbalance_score * weights['imbalance']) * 100
    else:
        total_weight = weights['completeness'] + weights['schema'] + weights['duplicates']
        score = ((missing_score * weights['completeness'] + schema_score * weights['schema'] + duplicate_score * weights['duplicates']) / total_weight) * 100
