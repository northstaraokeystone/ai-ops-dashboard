...
def test_create_and_verify_idempotency(client: TestClient, valid_payload: dict):
    ...
    assert first_response.status_code == 201, f"First submission failed: {first_response.text}"
    ...
    assert second_response.status_code == 200, "Idempotency check failed: expected 200 on second post"
