...
const CARD = "rounded-2xl bg-[#141414] border border-white/10 shadow-sm"; // radius ~20
...
export default function Panel1() {
  ...
  const [sourcesOpen, setSourcesOpen] = useState(false); // collapsed on load
  ...
  useEffect(() => {
    const onKey = (e) => {
      ...
      if (e.key.toLowerCase() === "e") exportAudit();
    };
  ...
  async function runAsk() {
    ...
    try {
      ...
      // map json → sources; setSources(mapped); setP95(Math.round(performance.now()-t0)); setCacheHit(1);
  ...
  function exportAudit() {
    if (!brief) return;
    downloadFile("dossier.json", JSON.stringify(brief, null, 2));
    downloadFile("receipts.jsonl", telemetry.join("\n") || '{"note":"telemetry sample"}');
  }
      ...
      <header className="sticky top-0 z-10 border-b border-white/10 bg-[#0E0E0E]/80 backdrop-blur">
        <div className="mx-auto max-w-6xl px-4 py-3 flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center gap-3 text-sm">
            ...
            <button className={`text-xs ${PILL_FOCUS}`} onClick={()=>setActiveTab("INSPECTOR")}>Receipts ✓</button>
            <div className="ml-3 flex gap-2">
              <Button onClick={redo} label="Redo" />
              <Button onClick={runBrief} label="Refine" />
              <Button onClick={exportAudit} label="Export" />
...
function Button({ label, onClick }) {
  return <button onClick={onClick} className={`px-3 py-2 bg-white/10 hover:bg-white/15 rounded-lg text-sm ${PILL_FOCUS}`}>{label}</button>;
}
