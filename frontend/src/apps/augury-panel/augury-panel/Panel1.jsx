// frontend/src/apps/augury-panel/Panel1.jsx
...
const API_BASE = import.meta.env.VITE_API_BASE ?? ""; // when you wire live

/* ---------- tiny ui atoms ---------- */
function Pill({ label }) {
  return <span className="px-2 py-1 bg-white/10 rounded-lg text-xs">{label}</span>;
}
function Button({ label, onClick }) {
  return (
    <button onClick={onClick} className={`px-3 py-2 bg-white/10 hover:bg-white/15 rounded-lg text-sm ${PILL_FOCUS}`}>
      {label}
    </button>
  );
}
function Tab({ label, active, onClick }) {
  return (
    <button className={`${TAB} ${active ? "bg-white/10" : "bg-transparent hover:bg-white/5"}`} onClick={onClick}>
      {label}
    </button>
  );
}
...
export default function Panel1() {
  ...
  const [sourcesOpen, setSourcesOpen] = useState(false); // preview collapsed by default
  ...
  useEffect(() => {
    ...
    setSourcesOpen(true); // auto-open preview on first result
  ...
  async function runAsk() {
    ...
    try {
      ...
      // const json = await res.json(); // map → sources
      // setSources(mapped); setP95(Math.max(1, Math.round(performance.now() - t0))); setCacheHit(1);
    ...
    <div className="min-h-screen bg-[#0E0E0E] text-white">
      ...
      <header className="sticky top-0 z-10 border-b border-white/10 bg-[#0E0E0E]/80 backdrop-blur">
        <div className="mx-auto max-w-6xl px-4 py-3 flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center gap-3 text-sm">
            ...
            <button className={`text-xs ${PILL_FOCUS}`} onClick={() => setShowReceipts(true)}>
              Receipts ✓
            </button>
      ...
      <section className="mx-auto max-w-3xl px-4 pt-8">
        <div className={`${CARD} p-4`}>
          ...
          <div className="mt-2 text-xs">
            {cacheHit ? <span className="text-green-400">Cached</span> : "Warm-up suggested"} · Press{" "}
            <kbd className="px-1 border rounded">/</kbd> to focus
          </div>
        </div>
      </section>
      ...
      <main className="mx-auto max-w-6xl px-4 py-6 grid grid-cols-12 gap-6">
        ...
        <div className="col-span-12 lg:col-span-8">
          ...
          {/* Sources tab: always render content */}
          {activeTab === "SOURCES" && (
            <div className={`${CARD} p-4`}>
              {!sources.length && <div className={`${MUTED} text-sm`}>Ask to load evidence. No raw PII.</div>}
              {!!sources.length && (
          ...
          {activeTab === "INSPECTOR" && (
            <div className={`${CARD} p-4`}>
              ...
              <pre className="text-xs whitespace-pre-wrap leading-5">
                {telemetry.join("\n") || "Run an action to view telemetry."}
              </pre>
            </div>
          )}
        </div>

        {/* Right preview dock (top-3) with working collapse */}
        ...
        <div role="dialog" aria-modal="true" className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center">
          <div className={`${CARD} p-5 w-full max-w-lg`}>
            <div className="flex items-center justify-between mb-3">
              ...
              <button className={`text-xs ${PILL_FOCUS}`} onClick={() => setShowReceipts(false)}>
                Close
              </button>
            </div>
            <pre className="text-xs whitespace-pre-wrap leading-5">
{JSON.stringify(brief?.receipts ?? {}, null, 2)}
      ...
      {/* tiny toast */}
      {toast && (
        <div className="fixed bottom-4 right-4 z-50 px-3 py-2 bg-white/10 border border-white/15 rounded-lg text-sm">
          {toast}
        </div>
      )}
    </div>
          ...
          <div className="flex items-start gap-2">
            ...
            <div>
              ...
              {typeof pt.confidence === "number" && (
                <div className="text-xs text-white/60 mt-1">confidence {(pt.confidence * 100).toFixed(0)}%</div>
              )}
            </div>
          </div>
        </section>
...
function SourcesDock({ sources, open, setOpen }) {
  return (
    <div className={`${CARD} p-4`}>
      <div className="flex items-center justify-between">
        <div className="text-sm font-medium">Sources</div>
        <button className={`text-xs ${PILL_FOCUS}`} onClick={() => setOpen(!open)}>
